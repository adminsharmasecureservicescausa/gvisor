...
func (s *rootSymlink) Readlink(ctx context.Context, _ *vfs.Mount) (string, error) {
	link, _, err := s.Getlink(ctx, nil)
	if err != nil {
		return "", err
	}
	defer s.fs.SafeDecRef(ctx, link)
	...
	vfsObj := link.Mount().Filesystem().VirtualFilesystem()
	name, _ := vfsObj.PathnameWithDeleted(ctx, root, link)
	return name, nil
}
